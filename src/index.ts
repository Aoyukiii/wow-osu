...
export const Config: Schema<Config> = Schema.object({
  client_id: Schema.string().required().description('osu! API 客户端 ID。'),
  client_secret: Schema.string().required().description('osu! API 客户端密钥。'),
})
...
export async function apply(ctx: Context, config: Config) {
  ...
  ctx
    .command('osu.user <username> [mode]', '获取玩家信息，默认模式为std。')
    .action(async ({ session }, username, mode) => {
      ...
      try {
        const user_details = await v2.users.details({
          user: username,
        ...
        const best_plays = await v2.scores.list({
          type: 'user_best',
...
async function loadSvgs() {
  ...
  const file_names = await fs.promises.readdir(dir, {
    withFileTypes: true,
  })
